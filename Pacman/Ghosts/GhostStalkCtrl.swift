//
//  GhostStalkCtrl.swift
//  Pacman
//
//  Created by Stian  Stensli on 10/11/17.
//  Copyright © 2017 Stian  Stensli. All rights reserved.
//

import Foundation

class GhostStalkCtrl : GhostCtrl{
    ...
    let spawn: ArrayNode = ArrayNode(xCor: 12, yCor: 10)
    ...
    override func move() -> MoveResult {
        ...
        if possibleDir.endIndex == 1 && isNotInSpawn && !super.reCalculate{
            ...
            let pacDir = goalLink.getDir()
            let pacx = goalLink.getNode().getX()
            let pacy = goalLink.getNode().getY()
            let frontNode = GameFunction.getNodeWithDir(xCor: pacx, yCor: pacy, dir: pacDir)
            
            let restrictNodes:[ArrayNode] = [frontNode, revertNode]
