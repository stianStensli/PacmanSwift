...
class GhostInterveneCtrl : GhostCtrl{
    ...
    let spawn: ArrayNode = ArrayNode(xCor: 14, yCor: 10)
    ...
    override func move() -> MoveResult {
        ...
        if possibleDir.endIndex == 1 && isNotInSpawn && !super.reCalculate{
            ...
            let pacOppDir = GameFunction.getOppDir(dir: goalLink.getDir())
            let pacx = goalLink.getNode().getX()
            let pacy = goalLink.getNode().getY()
            
            let backNode = GameFunction.getNodeWithDir(xCor: pacx, yCor: pacy, dir: pacOppDir)
            
            let restrictNodes:[ArrayNode] = [backNode, revertNode]
